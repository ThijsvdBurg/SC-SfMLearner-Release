...
def main():
    ...
    for j in tqdm(range(len(test_files))):
        ...
        ######################## VISUALIZE DEPTH #####################################################################
        depth=predictions[j]
        print('depthshape is ', depth.shape)
        depth_transpose = np.transpose(depth, (1, 2, 0))
        print('depth_transpose is ', depth_transpose.shape)
        imsave(output_dir/'{}_depth_640_480.png'.format(j),depth_transpose)
        ##############################################################################################################
